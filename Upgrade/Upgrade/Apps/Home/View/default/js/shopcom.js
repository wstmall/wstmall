					...
					html.push('<tbody class="j-order-row">');
						...
						for(var j=0;j<order.goodslist.length;j++){
							...
							html.push('<tr class="goods-box">');
								if(j==0){
									html.push("<td width='20' rowspan='"+order.goodslist.length+"'><input type='checkbox' class='chk_"+order.orderStatus+"' value='"+order.orderId+"'/></td>");
								}
								...
								if(j==0){
									...
									html.push('<td width="120" rowspan="'+order.goodslist.length+'">');  
										html.push('<div style="color:'+((order.orderStatus==-1 || order.orderStatus==-3 || order.orderStatus==-6 || order.orderStatus==-9)?'red':'')+';">¥'+order.realTotalMoney+'</div>');        
									html.push('</td>');
