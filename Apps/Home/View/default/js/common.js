	...
	jQuery.post(rooturl+"/index.php/Home/Cart/getCartInfo/" ,{"axm":1},function(data) {
		...
		for(var i=0;i<cart.cartgoods.length;i++){
			...
			html.push(  "<div style='float:left;'>" +
								"<a href='"+rooturl+"/index.php/Home/Goods/getGoodsDetails/?goodsId="+goods.goodsId+"'><img src='"+rooturl+"/"+goods.goodsThums+"' width='65' height='65'/></a>" +
						"</div>" +
						"<div style='float:left;width:280px;padding:4px;'>" +
								"<a target='_blank' href='"+rooturl+"/index.php/Home/Goods/getGoodsDetails/?goodsId="+goods.goodsId+"'>"+goods.goodsName+"</a><br/>￥"+goods.shopPrice+"元<br/>数量："+goods.cnt+"" +
...
function topay(){
	location.href=rooturl+"/index.php/Home/Cart/getCartInfo/?rnd="+Math.round(Math.random()*10000000);
}

function changeCity(areaId2){
	if(areaId2){
		jQuery.post(rooturl+"/index.php/?city="+areaId2 ,{"city":areaId2,"changeCity":true},function(data) {
		...
		location.href=rooturl+"/index.php/Home/Index/changecity";
	}
}

function addToFavorite(){
	var a=rooturl,b="\u4eac\u4e1cJD.COM-\u7f51\u8d2d\u4e0a\u4eac\u4e1c\uff0c\u7701\u94b1\u53c8\u653e\u5fc3";
