...
class AttributesModel extends BaseModel { 
	 ...
	 function edit(){
	 	...
	 	if(($priceAttrId>0 && $newPriceAttrId==0) || ($priceAttrId>0 && $newPriceAttrId>0 && $priceAttrId !=$newPriceAttrId )){
	 		//修改前一條记录状态
			$m->query("update __PREFIX__attributes set isPriceAttr=0 where attrId=".$priceAttrId);
	 ...
	 public function del(){
	    ...
	    //删除属性
	    $m = M('attributes');
	    $rs = $m->query("update __PREFIX__attributes set attrFlag=-1 where shopId=".$shopId." and attrId=".$id);
		if(false !== $rs){
		   $rd['status']= 1;
		}
		return $rd;
