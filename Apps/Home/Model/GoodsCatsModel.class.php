...
class GoodsCatsModel extends BaseModel {
	...
	*/
	...
	public function getGoodsCats($areaId2){
		$sql = "select * from ".$this->tablePrefix."goods_cats WHERE parentId = 0 AND isShow =1 AND catFlag = 1 order by catSort asc limit 6";
		$rs1 = $this->query($sql);
		$cats = array();
		for ($i = 0; $i < count($rs1); $i++) {
			$cat1Id = $rs1[$i]["catId"];
			$sql = "select * from ".$this->tablePrefix."goods_cats WHERE parentId = $cat1Id AND isShow =1 AND catFlag = 1 order by catSort asc";
			$rs2 = $this->query($sql);
			$cats2 = array();
			for ($j = 0; $j < count($rs2); $j++) {

				$cat2Id = $rs2[$j]["catId"];
				$sql = "select * from ".$this->tablePrefix."goods_cats WHERE parentId = $cat2Id AND isShow =1 AND catFlag = 1 order by catSort asc";
				$rs3 = $this->query($sql);
				$cats3 = array();
			...
			$obj["areaId2"] = $areaId2;
			$obj["goodsCatId1"] = $cat1Id;
			$recommendShops = self::getRecommendShops($obj);
			$rs1[$i]["recommendShops"] =$recommendShops;
	...
	public function getRecommendShops($obj){
		...
		$sql = "SELECT  COUNT(odr.orderId) as shopcnt, shop.* FROM ".$this->tablePrefix."shops shop
				LEFT JOIN ".$this->tablePrefix."orders odr ON shop.shopId = odr.shopId
